...
【關於這版：從 numpy 手刻 → PyTorch】
  ...
  Actor 直接複用 pg_cartpole 的 PolicyNetwork（同一個 nn.Module）。
  Critic 改成 nn.Module。Actor / Critic 各有一個 optimizer（兩種 lr）。

  autograd 帶來的一個關鍵簡化——detach：
    advantage = G_t - V(s)。更新 Actor 時，advantage 只是「權重」，
    不該讓梯度流回 Critic；更新 Critic 時，又要讓 V(s) 的梯度流動。
    PyTorch 用 .detach() 一行劃清界線：advantage 算給 Actor 時 detach 掉。
    （手刻 numpy 版是靠「Actor、Critic 各自獨立 backward」天然分開的。）
"""
...
class CriticNetwork(nn.Module):
    ...
    和 Actor 的差異：
      Actor  → 輸出 2 個 logits（softmax 後是機率，加總為 1）
    ...
    def forward(self, x):
        return self.net(x).squeeze(-1)   # (T, 1) → (T,)

    @torch.no_grad()
    def predict_value(self, state):
        """單筆推論：numpy 狀態 → float V(s)（給截斷 bootstrap 用）"""
...
class ACAgent:
    ...
    def update(self):
        ...
        流程：
          ...
          3. Advantage = G_t - V(s)  ← 給 Actor 當權重，要 detach
...
def train():
    ...
    print("=" * 60)
    print("  RL Hello World 4 — Actor-Critic (A2C) [PyTorch]")
    print("=" * 60)

...
概念改動（相比 ppo_cartpole.py，唯一一個）：
  ...
  ratio / clip / GAE / critic →  PPO 主體一行不改
```
...
```
⚠️ 為了讓較難的環境收斂，本階段「明示」加了兩個非核心工程處理：
   a. advantage 正規化（Pendulum reward 尺度大，CartPole 版不需要）
   b. 更新次數 250 → 400（Pendulum 較難）
   這兩項和 entropy bonus 同類——標配但非核心，故不混進「連續動作」概念。
```

> **λ 開關（本階段最重要的實驗）：** 把 `gae_lambda` 從 0.95 改成 **0.0**，
> advantage 就退回階段 4b 的單步 TD。實測：即使 advantage 正規化照開、
> 更新次數照給 400，它**依然學不起來**（卡在 ≈ -1300~-1500）。
> ⇒ 證明讓 Pendulum 能學的決定性因素是 **GAE**，不是正規化、也不是多跑幾次。
...
>
> **階段 5b（Pendulum）的兩個額外鬆綁**：它連 lr 都對齊（0.001/0.005），把
> 「相對離散 PPO 只改了動作分布」做到極致；但 Pendulum 較難，另外明示加了
> advantage 正規化、並把更新次數放寬到 400。這兩項屬「標配但非核心」的工程
> 處理（CartPole 用不到），刻意與「連續動作」這個概念分開講，不混進 diff。
